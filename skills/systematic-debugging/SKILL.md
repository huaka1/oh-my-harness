...
| "确认修复有效后再写测试" | 未经测试的修复不可靠。测试先证明它。 |
| "同时多个修复节省时间" | 无法隔离哪个生效了。会引起新 bug。 |
| "参考太长，我来适配模式" | 部分理解保证出 bug。完整阅读。 |
...
- **`condition-based-waiting.md`** - 用条件轮询替换武断超时
