...
创建或更新 `docs/harness/design/` 下的领域化设计文档树，使其反映实际构建了什么，而非初始计划猜测会发生什么。

设计文档不是 Agent 规则，也不是需求历史。它是模型理解项目事实背景的 map：先读入口索引，再按领域进入具体设计。
...
- 领域设计在 `docs/harness/design/<domain>/README.md` 和 `docs/harness/design/<domain>/<topic>.md`
...
4. **创建或更新领域化设计**
   - 永远先读取 `docs/harness/design/README.md`。如果不存在，创建它作为设计入口 map
   - 根据变更所属领域选择目录，例如 `browser/`、`auth/`、`agent-runtime/`、`cache/`、`operations/`
   - 如果领域不存在，创建 `docs/harness/design/<domain>/README.md` 作为领域索引
   - 更新对应领域下的主题文档，例如 `browser/sso-cookie-injection.md`、`auth/tgt-background-api.md`
...
这是项目设计文档入口。需要理解项目背景时，先读本文件，再按领域进入具体文档。

## 领域

- `browser/`：浏览器运行、CDP、用户隔离、SSO 注入
- `auth/`：用户身份、登录态、TGT 获取
...
- 要理解浏览器免登：读 `browser/README.md` → `browser/sso-cookie-injection.md` → `auth/tgt-background-api.md`
- 要理解缓存：读 `cache/README.md`
```
